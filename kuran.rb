...
VALID_PATH_RE  = /\A(surah|juz|page)\/[\d,]+(\/editions)?\/[\w.,\-]+\z/

# ───────────────────────────── YARDIMCI METODLAR ─────────────────────────────

# JSON yanıt yaz (Python'daki send_json karşılığı)
def send_json(data, status: 200)
  reasons = { 200 => 'OK', 400 => 'Bad Request', 502 => 'Bad Gateway' }
  reason  = reasons.fetch(status, 'OK')

  $stdout.write("Status: #{status} #{reason}\r\n")
...
# Dış API'ye istek gönder (Python'daki fetch_api karşılığı)
# SSL doğrulaması kapalı — PHP'deki CURLOPT_SSL_VERIFYPEER = false ile aynı
def fetch_api(url_str)
  uri  = URI.parse(url_str)
  http = Net::HTTP.new(uri.host, uri.port)
  http.use_ssl     = (uri.scheme == 'https')
  http.verify_mode = OpenSSL::SSL::VERIFY_NONE  # PHP/Python ile aynı davranış
  http.open_timeout = 20
  http.read_timeout = 20

  request = Net::HTTP::Get.new(uri.request_uri)
  request['User-Agent'] = 'DijitalKuran/1.0'
...
# API proxy — Python'daki handle_api karşılığı
def handle_api(path)
  unless VALID_PATH_RE.match?(path)
    send_json({ 'error' => 'Geçersiz API yolu' }, status: 400)
    return
  end

  url = "#{QURAN_API_BASE}/#{path}"
  begin
    body = fetch_api(url)
    ...
    send_json({ 'error' => 'API bağlantı hatası' }, status: 502)
  end
end

# HTML arayüzünü döndür — Python'daki handle_html karşılığı
def handle_html
  $stdout.write("Content-Type: text/html; charset=utf-8\r\n")
  $stdout.write("\r\n")
  $stdout.write(HTML_PAGE)
  $stdout.flush
end
...
function stripBasmala(text) {
  ...
  const isSkip = c => /[\\u064B-\\u065F\\u0670\\u06D6-\\u06ED\\u0640\\s\\u200F\\u200E]/.test(c);
...
def main
  # QUERY_STRING'i parse et (Python'daki get_query_params + urllib.parse.parse_qs karşılığı)
  cgi      = CGI.new
  api_path = (cgi['api'] || '').strip
